"""Step and timed playback for synchronized radar/camera snapshots."""
...
from processing.playback.playback import (
...
class SnapshotPlaybackController:
    def __init__(self, connection, pool, shutdown_event, initial_values):
        ...
        self.active = self.paused = self.stop_requested = False
        self.entries, self.index, self.current_reader = [], 0, None
    ...
    def _play(self, value):
        try:
            root, self.entries = _load_entries(value["folder"], value.get("synced_only", True))
            ...
            self._state(folder=str(root.resolve()))
            self._render()
            while self.active and not self.stop_requested and not self.shutdown_event.is_set():
                ...
                while self.active and not self.stop_requested and not self.shutdown_event.is_set():
                    ...
                    if self.connection.poll(wait):
                        ...
                        break
                    cv.waitKey(1)
            _send(self.pool, "snapshot_playback_state", {
                "active": False, "paused": False, "current": 0,
                "total": len(self.entries), "completed": not self.stop_requested,
            })
        except Exception as error:
            _send(self.pool, "snapshot_playback_error", str(error))
            _send(self.pool, "snapshot_playback_state", {
                "active": False, "paused": False, "current": 0, "total": 0,
            })
        finally:
            self.active = False
    ...
    def _frame_delay(self):
        if self.index + 1 >= len(self.entries):
            return 0.05
        return max(0.05, (self.entries[self.index + 1].recorded_at - self.entries[self.index].recorded_at).total_seconds())
    ...
    def _handle(self, event, value):
        if event == "STOP":
            ...
            self._state()
        elif event in ("snapshot_playback_next", "snapshot_playback_previous") and self.entries:
            self.paused = True
    ...
    def _render(self):
        ...
        _send(self.pool, "snapshot_playback_progress", {
    ...
    def _state(self, **extra):
        _send(self.pool, "snapshot_playback_state", {
            ...
            "total": len(self.entries), **extra,
        })

    def _save_snapshot(self, destination):
        if not self.entries:
        ...
        camera_time = entry.camera_recorded_at or entry.recorded_at + timedelta(seconds=self.camera_delay_seconds)
        result = ManualSnapshotWriter(destination, self.camera_delay_seconds).save(
