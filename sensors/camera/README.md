...
recording, and temporarily changes channels for manual snapshots.
...
group, capture one frame, and then restore the previous channel.
...
- Latency adjustment, provisional default 87.348 ms, is an application-level camera-to-radar
  alignment offset used by recording, snapshots, and playback snapshots.

Changing one does not automatically derive the other. The adjustment should be
validated for the current stream session and hardware path.
...
across recording boundaries. Reconnects and camera configuration changes still rebuild the pipeline.
...
final recording summary. These are RTP packet counters, not decoded-frame
